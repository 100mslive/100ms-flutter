...
import 'package:live_emoji_reaction/meeting/item.dart';
import 'package:live_emoji_reaction/meeting/peer_track_node.dart';
import 'package:live_emoji_reaction/utilities.dart';
...
class _MeetingPageState extends State<MeetingPage>
  ...
  //List of item for raining emoji
  var items = <Item>[];

  //Emoji reaction animation controller
  late AnimationController animationController;

  List<String> emojiIconList = ["☺️", "👍", "❤️", "🥰", "🤩"];

  //emojiReaction function create new emoji and reset animationController.
  emojiReaction(String message) async {
    setState(() {
      items.clear();
      for (int i = 0; i < 100; i++) {
        items.add(Item(emoji: message));
      }
    });
  ...
  //scroll to bottom at chat window
  _scrollToBottom() {
    scrollController.jumpTo(scrollController.position.maxScrollExtent);
  }

  // Building emoji and adding animation and different size to them
  List<Widget> buildEmojiWidget() {
      ...
      return SlideTransition(
        ...
        child: AnimatedAlign(
          ...
          child: AnimatedContainer(
            ...
            child: Text(
              item.emoji,
              style: TextStyle(fontSize: item.size.toDouble()),
            ),
          ),
        ),
      );
              ...
              Padding(
                ...
                child: Align(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                    children: [
                      for (int i = 0; i < emojiIconList.length; i++)
                        GestureDetector(
                          onTap: () {
                            _hmssdk.sendBroadcastMessage(
                                message: emojiIconList[i],
                                type: "emoji",
                                hmsActionResultListener: this);
                          },
                          child: Container(
  ...
  void onException(
    ...
    print(
        "Error: type: ${methodType.name} hmsException: ${hmsException.message} arguments:$arguments");
  }
  ...
  void onSuccess(
    ...
    switch (methodType) {
      ...
      case HMSActionResultListenerMethod.sendBroadcastMessage:
        ...
        if (arguments!['type'] != null && arguments['type'] == "emoji") {
          emojiReaction(arguments['message']);
          break;
        }
        ...
        break;
      default:
        print("Success: type: ${methodType.name} arguments:$arguments");
        break;
    }
  }
